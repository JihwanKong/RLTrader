...
class Agent:
    # AGENT 상태가 구성하는 값 개수
    STATE_DIM = 2  # 주식 보유 비율, 포트폴리오 가치비율

    TRADING_CHARGE = 0.00015  # 거래 수수료(보통 0.015%)
    TRADING_TAX = 0.0025  # 거래세
    # TRADING_CHARGE = 0  # 거래 수수류 미적용
    ...
    def __init__(self, environment, min_trading_unit,
    ...
    def get_states(self):
        ...
        # 포토폴리오 가치 비율  = 포트폴리오 가치/기준 포트폴리오 가치
    ...
    def act(self, action, confidence):
        ...
        if action == Agent.ACTION_BUY:
            ...
            if balance < 0:
                trading_unit = max(
                    min(int(self.balance / (curr_price * (1 + self.TRADING_CHARGE))),
        ...
        if self.base_profitloss > self.delayed_reward_threshold or \
            ...
            # 혹은 손실 기준치를 초과하여 기준 포트폴리오 가치 갱신
