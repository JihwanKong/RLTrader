...
class ReinforcementLearner:
    ...
    @abc.abstractmethod  # ReinforcedLearner의 하위 클래스들은 get_batch를 무조건 구현해야함
    def get_batch(self, batch_size, delayed_reward, discount_factor):
        pass

    def update_networks(self, batch_size,
                        delayed_reward, discount_factor):
        # 배치 학습데이터 생성
        ...
        if len(x) > 0:
            ...
            if y_value is not None:
                # 가치 신경망 갱신
                loss += self.value_network.train_on_batch(x, y_value)  # feature, 가치 값으로 학습
            if y_policy is not None:
                # 정책 신경망 갱신
                loss += self.policy_network.train_on_batch(x, y_policy)  # feature, 정책 값으로 학습
            return loss
    ...
    def visualize(self, epoch_str, num_epoches, epsilon):
        ...
        self.visualizer.save(os.path.join(
            self.epoch_summary_dir,  # epoch summary directory 정해야함
...
class DQNLearner(ReinforcementLearner):
    def __init__(self, *args, value_network_path=None, **kwargs):
        ...
        # self.init_value_network(value_network_path=value_network_path)
    ...
    def get_batch(self, batch_size, delayed_reward, discount_factor):
        ...
        for i, (sample, action, value, reward) in enumerate(memory):
            x[i] = sample
            y_value[i] = value
            # 마지막 손익률 보다 현재 손익률이 크다. -> action울 마지막 까지 하면 안됨 -> value 하락
            # 다음 손익률 보다 현재 손익률이 크다. -> next action을 하면 안됨 -> value 하락
...
class A3CLearner(ReinforcementLearner):
    ...
    # abstract method 정의로 인해 기능이 없어도 정의 혹은 abstract method 정의 부분을 상속하여야 함
    def get_batch(self, batch_size, delayed_reward, discount_factor):
        pass
